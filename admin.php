...
echo"<hr>";
	...
	$res=mysql_query("SELECT * FROM `airline_fares` WHERE `from`= '$from' && `to`= '$to'") or die(mysql_error());
	while($row=mysql_fetch_array($res))
	{
				
			$code=$row[8];
			$_SESSION['code']=$code;
			echo "<a href=status.php?resid=$code>";echo "<table>";
			...
			echo "<tr><td><img src='".$row[0].".jpg' height=100 width=90 /></td><td>".$row[1]."</td><td>".$row[2]."</td><td>".$row[3]."</td><td>".$row[4]."</td><td>".$row[5]."</td><td>".$row[9]."</td></tr>";
